...
use cosmwasm_std::{
    to_binary, Addr, Coin, Decimal, Empty, Querier, Uint128,
};
...
use crate::test_helpers::constants::UOSMO;
...
impl<'a, R: Runner<'a> + Querier> OsmosisVaultRobot<'a, R> {
    ...
    pub(crate) fn deposit_all(&self, signer: &SigningAccount, recipient: Option<String>) -> &Self {
        let balance = self.query_base_token_balance(&signer.address());
        self.deposit(signer, recipient, balance)
    }

    pub(crate) fn unlock_all(&self, signer: &SigningAccount) -> &Self {
        let balance = self.query_vault_token_balance(&signer.address());
        self.unlock(signer, balance)
    }

    pub(crate) fn vault_token(&self) -> String {
        self.query_state().vault_token.to_string()
    }

    pub(crate) fn unlock(&self, signer: &SigningAccount, amount: Uint128) -> &Self {
        let unlock_msg =
    ...
    pub(crate) fn query_unlocking_positions(&self, address: &str) -> Vec<UnlockingPosition> {
        let wasm = Wasm::new(self.app);
        wasm.query(
            &self.vault_addr,
    ...
    pub(crate) fn withdraw_first_unlocked(
        &self,
        signer: &SigningAccount,
        recipient: Option<String>,
    ) -> &Self {
    ...
    pub(crate) fn base_token(&self) -> String {
        self.base_pool.lp_token().to_string()
    }

    pub(crate) fn assert_total_staked_base_tokens(&self, expected: Uint128) -> &Self {
        let state = self.query_state();
    ...
    pub(crate) fn assert_vault_token_supply(&self, expected: Uint128) -> &Self {
        let state = self.query_state();
    ...
    pub(crate) fn assert_vault_token_share(&self, address: &str, expected: Decimal) -> &Self {
    ...
    pub(crate) fn simulate_reward_accrual(
        &self,
        signer: &SigningAccount,
        reward_denom: &str,
        amount: Uint128,
...
impl<'a> OsmosisVaultRobot<'a, OsmosisTestApp> {
    pub(crate) fn increase_time(&self, duration: Duration) -> &Self {
